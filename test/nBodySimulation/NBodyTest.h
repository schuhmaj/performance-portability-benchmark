...
protected:
    ...
    */
    ...
    double get_mean_deviation(const std::vector<ppb::Particle<float>>& expected, const std::vector<ppb::Particle<float>>& actual, size_t size) {
        double mean_deviation = 0.f;

        for (size_t i = 0; i < size; i++) {
            double dev_x = std::abs(expected[i].getPosition()[0] - actual[i].getPosition()[0]);
            double dev_y = std::abs(expected[i].getPosition()[1] - actual[i].getPosition()[1]);
            double dev_z = std::abs(expected[i].getPosition()[2] - actual[i].getPosition()[2]);
    ...
    double get_mean_deviation_euclidean(const std::vector<ppb::Particle<float>>& expected, const std::vector<ppb::Particle<float>>& actual, size_t size) {
        double mean_deviation = 0.f;

        for (size_t i = 0; i < size; i++) {
            double dev_x = (expected[i].getPosition()[0] - actual[i].getPosition()[0]);
            double dev_y = (expected[i].getPosition()[1] - actual[i].getPosition()[1]);
            double dev_z = (expected[i].getPosition()[2] - actual[i].getPosition()[2]);
            double euclidean = std::sqrt(dev_x*dev_x + dev_y*dev_y + dev_z*dev_z);
            mean_deviation += euclidean;
            std::cout<<euclidean<<","<<std::endl;
        }

        return mean_deviation / size;
    }

    double get_rmse(const std::vector<ppb::Particle<float>>& expected, const std::vector<ppb::Particle<float>>& actual, size_t size) {
        double acc = 0.f;
        ...
        for (size_t i = 0; i < size; i++) {
            ...
            double dev_z = (expected[i].getPosition()[2] - actual[i].getPosition()[2]);
            double euclidean = std::sqrt(dev_x*dev_x + dev_y*dev_y + dev_z*dev_z);
            acc += euclidean * euclidean;
        }

        return std::sqrt(acc / size);
    }

    double get_median_deviation(const std::vector<ppb::Particle<float>>& expected, const std::vector<ppb::Particle<float>>& actual, size_t size) {
...
/*         ParticleSimulationConfig<float> config{static_cast<size_t>(size), 1, 0.0005};
        ...
        for (size_t i = 0; i < ITERATIONS; i++) { 
            float total_energy_before = get_total_energy(stateActual);
            ParticleSimulationConfig<float> config{static_cast<size_t>(size), 1, 0.0005};
            NBodySimulation<ImplCpp<float>> cppNBodySim{config};
            NBodySimulation<Implementation> otherNBodySim{config};
            ...
            float total_energy_after = get_total_energy(stateActual);
            if (i % 10 == 0) {
                std::cout<<"BEFORE: "<<total_energy_before<<"AFTER: "<<total_energy_after<<std::endl;
                std::cout<<std::abs((total_energy_before - total_energy_after) / total_energy_before)<<std::endl;
                std::cout<<get_rmse(stateExpected, stateActual, static_cast<size_t>(size))<<std::endl;
            }
            if (i == 1 || i == 10 || i == 100 || i == 1000) {
                printf("i = %lu\n", i);
                get_mean_deviation_euclidean(stateExpected, stateActual, static_cast<size_t>(size));
            }
        } */
    }
};
...
protected:
     ...
     */
    static constexpr int LATTICE_SIZE_X = 10;
    static constexpr int LATTICE_SIZE_Y = 10;
    static constexpr int LATTICE_SIZE_Z = 10;
     ...
     */
    static constexpr double BOX_HALF_WIDTH = 30.0;
...
/*         ParticleSimulationConfig<float> config_numerical_stability{initialParticles.size(), 1,
                                               static_cast<float>(ENERGY_TIME_STEP)}; */
        
        Implementation nBodySim{config};
        const auto [finalParticles, timings] = nBodySim.simulate(initialParticles);

/*         std::vector<ppb::Particle<float>> startParticles = finalParticles;
        for(size_t i = 0; i < ENERGY_ITERATIONS; i++) {
            double energyBefore = kineticEnergy(startParticles) + potentialEnergy(startParticles);     
            Implementation nBodySim{config_numerical_stability};
            auto [endParticles, timings] = nBodySim.simulate(startParticles);  
            double energyAfter = kineticEnergy(endParticles) + potentialEnergy(endParticles); 
            startParticles = endParticles;
            finalParticles = endParticles;
            if (i % 50 == 0) {
                std::cout<<std::abs(energyAfter - energyBefore) / std::abs(energyBefore)<<std::endl;
            }  
        }
 */

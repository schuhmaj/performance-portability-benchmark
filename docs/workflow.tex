...
% them. Build the PDF and the PNG with:
%
%   make -C docs
%
% ---------------------------------------------------------------------------
...
% Performance-portability symbol, identical to the definition used in the paper.
\newcommand{\PP}{\ensuremath{\mathord{\mbox{\reflectbox{P}\kern-0.3em P}}}}
...
\begin{tikzpicture}[
    ...
    \node[stage title, text=white, font=\sffamily\bfseries\normalsize] at (10.80,9.45)
        {Performance portability \PP{} and Code Complexity};
    ...
    \begin{scope}[shift={(16.50,0)}]
        ...
        \node[mono] at (0.28,4.35) {ppbcc p3analysis combined \(\backslash\)};
        \node[mono] at (0.28,4.03) {~~complexity.csv Results\_* \dots};
        \node[desc] at (0.28,3.61) {Application efficiency \(e_A\) and};
        \node[desc] at (0.28,3.29) {performance portability \PP{} over};
    ...
    \foreach \cx/\fillc/\chip in {%
        ...
        16.50/repoPpbccLight/{Chart PDFs, \(e_A\) and \PP{} as CSV}%
    } {

...
__global__ void kernel_vector_add(int size, FloatType *__restrict__ a, FloatType *__restrict__ b,
...
std::vector<FloatType> VectorAddition<FloatType>::operator()() {
    ...
    kernel_vector_add<<<numBlocks, threadsPerBlock, 0, stream>>>(_inA.size(), deviceA, deviceB, deviceC);
...
template __global__ void kernel_vector_add<float>(int, float *__restrict__, float *__restrict__, float *__restrict__);
template std::vector<float> VectorAddition<float>::operator()();
BENCHMARK(VectorAddition<float>::vectorAdditionBenchmark)
    ->Name("VecAdd-Cuda2-Float")
    ->RangeMultiplier(10)
    ->Range(1e3, 1e8)
    ->Complexity();

template __global__ void kernel_vector_add<double>(int, double *__restrict__, double *__restrict__,
                                                   double *__restrict__);
template std::vector<double> VectorAddition<double>::operator()();
BENCHMARK(VectorAddition<double>::vectorAdditionBenchmark)
    ->Name("VecAdd-Cuda2-Double")
    ->RangeMultiplier(10)
    ->Range(1e3, 1e8)
    ->Complexity();

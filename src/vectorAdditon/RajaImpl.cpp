...
RAJA::resources::Resource resource{RAJA::resources::Cuda()};
#else
using exec_policy = RAJA::seq_exec;
RAJA::resources::Resource resource{RAJA::resources::Host()};
#endif
...
std::vector<FloatType> VectorAddition<FloatType>::operator()() {
    ...
    RAJA::forall<exec_policy>(RAJA::RangeSegment(0, _inA.size()), [=] RAJA_HOST_DEVICE (int i) {
...
BENCHMARK(VectorAddition<float>::vectorAdditionBenchmark)->Name("VecAdd-RAJA-Float")->RangeMultiplier(10)->Range(1e3, 1e8)->Complexity();

// Instantiate a benchmark using double precision
template std::vector<double> VectorAddition<double>::operator()();
BENCHMARK(VectorAddition<double>::vectorAdditionBenchmark)->Name("VecAdd-RAJA-Double")->RangeMultiplier(10)->Range(1e3, 1e8)->Complexity();

...
public:
    GravityEvaluable(
        ...
        : GravityEvaluableBase(Vertices, Faces, density), _normals(Faces.size()), _results(Faces.size()) {
    }
    ...
    GravityModelResult evaluate(const Array3 &Point) override {
        ...
        result = std::accumulate(_results.begin(), _results.end(), result);
...
private:
    ...
    void init() {
        ...
        for (size_t i = 0; i < _faces.size(); ++i) {
            const Array3Triplet face = {_vertices[_faces[i][0]], _vertices[_faces[i][1]], _vertices[_faces[i][2]]};

 ...
 *
 * nvc++ -stdpar=gpu (NVHPC 26.3) miscompiles zero-initialized struct constants inside the offloaded lambdas
 * (see PolyhedralGravityDefinitions.h), so zero locals are written as `Array3 x{}` and never as `{0.0, 0.0, 0.0}`.
 */
    ...
    void init() {
        ...
        std::for_each(std::execution::par_unseq, normals, normals + faceCount, [=](Array3 &faceNormal) {
            ...
            const Array3Triplet face = {
                    ...
                    vertices[faces[i][1]],
                    vertices[faces[i][2]]};
            faceNormal = normal(face[1] - face[0], face[2] - face[1]);
        });

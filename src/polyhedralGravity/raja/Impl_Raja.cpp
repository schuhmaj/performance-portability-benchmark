...
GlobalResources::GlobalResources(int &argc, char *argv[]) {
    // Initialize the device context/ queue up-front, so the first kernel launch is not penalized
    Resource res = Resource::get_default();
    auto *dummy = res.allocate<double>(1);
    res.deallocate(dummy);
    res.wait();
}
...
public:
    GravityEvaluable(
        ...
        : GravityEvaluableBase(Vertices, Faces, density),
          _res{Resource::get_default()},
          _deviceVertices{_res.allocate<Array3>(Vertices.size())},
          _deviceFaces{_res.allocate<IndexArray3>(Faces.size())},
          _normals{_res.allocate<Array3>(Faces.size())},
          _segmentVectors{_res.allocate<Array3Triplet>(Faces.size())},
          _segmentNormals{_res.allocate<Array3Triplet>(Faces.size())} {
    }

    ~GravityEvaluable() override {
        _res.deallocate(_deviceVertices);
        _res.deallocate(_deviceFaces);
        _res.deallocate(_normals);
        _res.deallocate(_segmentVectors);
        _res.deallocate(_segmentNormals);
    }
    ...
    GravityModelResult evaluate(const Array3 &Point) override {
        ...
        const Array3Triplet *segmentVectors = _segmentVectors;
        const Array3Triplet *segmentNormals = _segmentNormals;
    ...
    void init() {
        ...
        RAJA::forall<ExecPolicy>(_res, RAJA::TypedRangeSegment<size_t>(0, _faces.size()), [=] RAJA_HOST_DEVICE(const size_t i) {
            ...
            segmentVectors[i] = {Face[1] - Face[0], Face[2] - Face[1], Face[0] - Face[2]};
            normals[i] = normal(segmentVectors[i][0], segmentVectors[i][1]);
            segmentNormals[i] = {
                    normal(segmentVectors[i][0], normals[i]),
                    normal(segmentVectors[i][1], normals[i]),
                    normal(segmentVectors[i][2], normals[i]),
            }; });
        _res.wait();
...
private:
    Resource _res;
    Array3 *_deviceVertices;
    IndexArray3 *_deviceFaces;
    Array3 *_normals;

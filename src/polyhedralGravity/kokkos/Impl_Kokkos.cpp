...
// On the CUDA backend the launch bounds become __launch_bounds__(256, 3): at most 256 threads per block, and a body
// small enough for three such blocks per SM, so that the register allocator keeps the kernel's occupancy up.
// The host backends ignore them.
using EvaluationPolicy = Kokkos::RangePolicy<Kokkos::LaunchBounds<256, 3>>;
...
public:
    ...
    GravityModelResult evaluate(const Array3 &Point) override {
        ...
        const Kokkos::View<Array3 *> vertices = _deviceVertices;
        const Kokkos::View<IndexArray3 *> faces = _deviceFaces;
        const Kokkos::View<Array3 *> normals = _Normals;

        GravityModelResult result{};

        Kokkos::parallel_reduce("Execute", EvaluationPolicy(0, _faces.size()), KOKKOS_LAMBDA(const size_t i, GravityModelResult &accumulator) {
            ...
            for (unsigned int index = 0; index < 3; ++index) {
                ...
                segmentDistances[index] = Kokkos::abs(normalProjection);
                ...
                if (Kokkos::abs(distance.s1 - distance.l1) >= EPSILON_ZERO || Kokkos::abs(distance.s2 - distance.l2) >= EPSILON_ZERO) {
                ...
                } else if (Kokkos::abs(distance.s2 - distance.s1) < EPSILON_ZERO) {
            ...
            for (unsigned int index = 0; index < 3; ++index) {
                ...
                const bool logarithmVanishes =
                        ...
                        (Kokkos::abs(distance.s1 + distance.s2) < EPSILON_ZERO && Kokkos::abs(distance.l1 + distance.l2) < EPSILON_ZERO);
            ...
            if (anyAtVertex) {
                ...
                vertexAngle = gdot == 0 ? PI_2 : Kokkos::acos(gdot / (euclideanNorm(g1) * euclideanNorm(g2)));
            }
    ...
    void init() {
        ...
        Kokkos::parallel_for("RunInit", _faces.size(), KOKKOS_LAMBDA(const size_t i) {
            const Array3Triplet face = {vertices(faces(i)[0]), vertices(faces(i)[1]), vertices(faces(i)[2])};

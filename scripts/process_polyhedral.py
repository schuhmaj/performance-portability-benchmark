...
def create_dataframe(report_files: list[Path]) -> pd.DataFrame:
    ...
    for file in report_files:
        ...
        for col in ["kernel_time", "position_update_reset", "velocity_update", "force_update", "Version"]:
        ...
        if "Problem Size" in df.columns:
            df["Problem Size"] = pd.to_numeric(df["Problem Size"], errors="coerce").astype("Int64")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", nargs='+', type=Path, default=[Path.cwd()], help="Path to search for files")
